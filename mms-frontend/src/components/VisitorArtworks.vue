<template>
<div>
  <h4 class = "page-description"> Welcome, {{ this.currentUserEmail }}! 
  </h4>
  ...
  <div class = "filters">
    ...
    <button @click="showPopup=true" class="all-artworks">New Request</button>
  </div>

  <div class="requestForm">
    <vs-popup class="holamundo"  title="New Request" :active.sync="showPopup">
      <table>
        <tr> 
          <td><input type="text" placeholder="Artwork ID" v-model="newArtworkID"></td>
          <td><input type="date" placeholder="Start Date" v-model="newStartDate"></td>
          <td><input type="date" placeholder="End Date" v-model="newEndDate"></td>
          <td><button v-bind:disabled="loanBtnDisabled" @click="createLoan(newStartDate, newEndDate, newArtworkID)">Create Request</button></td>
        </tr>
        <tr>
          <p v-if="createErrorMsg" style="color: red">
            {{ createErrorMsg }}
          </p>
          <p v-if="createSuccessMsg" style="color: green"> 
            {{ createSuccessMsg }}
          </p>
        </tr>
      </table>
...
export default {
  ...
  data() {
    return {
      ...
      roomID: '',
      artist: '',
      currentUserEmail: window.sessionStorage.getItem('email'),
      failiureMessage1: '',
      failiureMessage2: '',
      createErrorMsg: '',
      createSuccessMsg: '',
  ...
  methods: {
    ...
    createLoan: function(newStartDate, newEndDate, newArtworkID) {
      AXIOS.post('/loan', {
        startDate: newStartDate,
        endDate: newEndDate,
        loanRequestorID: window.sessionStorage.getItem('visitorID'),
    ...
    computed: { // conditions for disabling button
      loanBtnDisabled() {
        return !this.newStartDate.trim() || !this.newEndDate.trim() || !this.newArtworkID.trim()
      }
    },
  }
};
